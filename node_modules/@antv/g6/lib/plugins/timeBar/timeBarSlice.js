...
var _timeBarTooltip = _interopRequireDefault(require("./timeBarTooltip"));
  ...
  TimeBarSlice.prototype.initEvent = function () {
    ...
    sliceGroup.on('click', function (e) {
      ...
      if (!isNaN(id)) {
        ...
        _this.graph.emit(_constant.VALUE_CHANGE, {
          value: [start, start]
        });
      }
    });
    ...
    var tooltip = new _timeBarTooltip.default({
    ...
    group.on(_constant.PLAY_PAUSE_BTN + ":click", function () {
      _this.isPlay = !_this.isPlay;

      _this.changePlayStatus();
    }); // 处理前进一步的事件

    group.on(_constant.NEXT_STEP_BTN + ":click", function () {
      _this.updateStartEnd(1);
    }); // 处理后退一步的事件

    group.on(_constant.PRE_STEP_BTN + ":click", function () {
      _this.updateStartEnd(-1);
    });
    group.on(_constant.TIMEBAR_CONFIG_CHANGE, function (_a) {
      var type = _a.type,
          speed = _a.speed;
