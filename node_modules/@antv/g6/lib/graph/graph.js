...
var _stack = _interopRequireDefault(require("../algorithm/structs/stack"));

var _adjacentMatrix = _interopRequireDefault(require("../algorithm/adjacent-matrix"));

var _floydWarshall = _interopRequireDefault(require("../algorithm/floydWarshall"));
...
function (_super) {
  ...
  function Graph(cfg) {
    ...
    if (_this.cfg.enabledStack) {
      // 实例化 undo 和 redo 栈
      _this.undoStack = new _stack.default(_this.cfg.maxStep);
      _this.redoStack = new _stack.default(_this.cfg.maxStep);
    }
  ...
  Graph.prototype.translate = function (dx, dy) {
    ...
    _matrixUtil.mat3.translate(matrix, matrix, [dx, dy]);
  ...
  Graph.prototype.zoom = function (ratio, center) {
    ...
    if (center) {
      _matrixUtil.mat3.translate(matrix, matrix, [-center.x, -center.y]);

      _matrixUtil.mat3.scale(matrix, matrix, [ratio, ratio]);
    ...
    (comboTrees || []).forEach(function (ctree) {
      (0, _graphic.traverseTreeUp)(ctree, function (child) {
        ...
        if (comboId === child.id && childItem && childItem.getType && childItem.getType() === 'combo') {
          // 更新具体的 Combo 之前先清除所有的已有状态，以免将 state 中的样式更新为 Combo 的样式
          var states = (0, _tslib.__spreadArrays)(childItem.getStates()); // || !item.getStateStyle(stateName)
  ...
  Graph.prototype.updateComboTree = function (item, parentId, stack) {
    if (stack === void 0) {
      stack = true;
    }
  ...
  Graph.prototype.toFullDataURL = function (callback, type, imageConfig) {
    ...
    var renderer = this.get('renderer');
    var vContainerDOM = (0, _domUtil.createDom)('<id="virtual-image"></div>');
    ...
    _matrixUtil.mat3.translate(matrix, matrix, [-centerX, -centerY]);

    _matrixUtil.mat3.translate(matrix, matrix, [width / 2 + padding[3], height / 2 + padding[0]]);
  ...
  Graph.prototype.getAdjMatrix = function (cache, directed) {
    ...
    if (!currentAdjMatrix || !cache) {
      currentAdjMatrix = (0, _adjacentMatrix.default)(this, directed);
  ...
  Graph.prototype.getShortestPathMatrix = function (cache, directed) {
    ...
    if (!currentShourtestPathMatrix || !cache) {
      currentShourtestPathMatrix = (0, _floydWarshall.default)(this, directed);
  ...
  Graph.prototype.on = function (eventName, callback, once) {
