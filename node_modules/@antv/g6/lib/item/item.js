...
function () {
  ...
  ItemBase.prototype.setOriginStyle = function (cfg) {
    ...
    var shapeFactory = _shape.default.getFactory(itemType)[shapeType];

    if (!shapeFactory) shapeFactory = _shape.default.getFactory(itemType).getShape();
    ...
    if (itemType === 'edge') {
      if (!defaultStyle.lineWidth) defaultStyle.lineWidth = size || _global.default.defaultEdge.size;
    } else {
      if (!defaultStyle.r) defaultStyle.r = size / 2 || _global.default.defaultNode.size / 2;
      if (!defaultStyle.width) defaultStyle.width = ((0, _util.isArray)(size) ? size[0] : size) || _global.default.defaultNode.size / 2;
      if (!defaultStyle.height) defaultStyle.height = ((0, _util.isArray)(size) ? size[1] : size) || _global.default.defaultNode.size / 2;
    }
    ...
    if (!keyShapeName) {
      ...
      for (var key in defaultStyle) {
        ...
        if (!(0, _util.isPlainObject)(style) || ARROWS.includes(key)) styles_1[key] = style;
      }

      if (styles_1.path) delete styles_1.path;
      if (styles_1.matrix) delete styles_1.matrix;
      if (!originStyles[keyShapeName]) originStyles[keyShapeName] = styles_1;else originStyles[keyShapeName] = Object.assign(styles_1, originStyles[keyShapeName]);
    }
    ...
    if (cfg) {
      styles = (0, _util.deepMix)({}, drawOriginStyle, originStyles, cfg.style, {
        labelCfg: cfg.labelCfg
      });
    } else {
      styles = (0, _util.deepMix)({}, drawOriginStyle, originStyles);
    }
