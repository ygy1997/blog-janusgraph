...
var shapeBase = {
    ...
    if (rotate) {
      ...
      if (labelStyle.rotateCenter) {
        switch (labelStyle.rotateCenter) {
          case 'center':
            labelMatrix = (0, _matrixUtil.transform)(labelMatrix, [['t', -labelBBox.width / 2, -labelBBox.height / 2], ['r', rotate], ['t', labelBBox.width / 2, labelBBox.height / 2]]);
            break;

          case 'lefttop':
            labelMatrix = (0, _matrixUtil.transform)(labelMatrix, [['t', -labelStyle.x, -labelStyle.y], ['r', rotate], ['t', labelStyle.x, labelStyle.y]]);
            break;

          case 'leftcenter':
            labelMatrix = (0, _matrixUtil.transform)(labelMatrix, [['t', -labelStyle.x, -labelStyle.y - labelBBox.height / 2], ['r', rotate], ['t', labelStyle.x, labelStyle.y + labelBBox.height / 2]]);
            break;
    ...
    if (cfg.label || cfg.label === '') {
      ...
      if (!label) {
        ...
        if (rotate) {
          ...
          rotateMatrix = (0, _matrixUtil.transform)(rotateMatrix, [['t', -labelStyle.x, -labelStyle.y], ['r', rotate], ['t', labelStyle.x, labelStyle.y]]);
          label.resetMatrix();
  ...
  setState: function setState(name, value, item) {
    ...
    if (value) {
      ...
      var keyShapeStyles_1 = (0, _util.clone)(shape.attr()); // 已有样式 - 要取消的状态的样式
      ...
      var _loop_4 = function _loop_4(originKey) {
        ...
        if ((0, _util.isPlainObject)(style) && !ARROWS.includes(originKey)) {
          ...
          if (subShape) {
            subShape.attr(style);
            if (originKey === keyShapeName_1) keyShapeSetted = true;
          }
          ...
          if (type === 'combo') {
            if (!keyShapeName_1) {
              shape.attr((_a = {}, _a[originKey] = value_4, _a));
            }
          } else {
            shape.attr((_b = {}, _b[originKey] = value_4, _b));
          }
        }
      };
